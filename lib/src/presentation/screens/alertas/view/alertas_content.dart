...
class AlertaContent extends StatelessWidget {
  ...
  final ValueChanged<AlertaDestinatarioModel> onAlertaTap;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            tooltip: 'Actualizar',
            onPressed: state.listStatus == AlertaListStatus.loading
                ? null
                : onRetry,
  ...
  Widget _buildBody(BuildContext context) {
    if (state.listStatus == AlertaListStatus.loading && state.alertas.isEmpty) {
    ...
    if (state.listStatus == AlertaListStatus.error && state.alertas.isEmpty) {
      return _AlertaErrorView(
        message: state.errorMessage ?? 'No se pudieron obtener las alertas.',
        onRetry: onRetry,
      );
    }
      ...
      child: NotificationListener<ScrollNotification>(
        ...
        child: CustomScrollView(
          ...
          slivers: [
              ...
              SliverPadding(
                ...
                sliver: SliverList.separated(
                  ...
                  itemBuilder: (context, index) {
                    final destinatario = state.alertas[index];

                    return _AlertaCard(
                      destinatario: destinatario,
                      onTap: () {
                        onAlertaTap(destinatario);
                      },
                    );
                  },
                ),
            ...
            if (state.listStatus == AlertaListStatus.loadingMore)
            ...
            if (!state.hasNextPage && state.alertas.isNotEmpty)
              const SliverToBoxAdapter(
                child: Padding(
                  padding: EdgeInsets.only(top: 4, bottom: 24),
                  child: Center(
...
class _AlertaResumenSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final noLeidas = resumen?.noLeidas ?? state.totalNoLeidas;
    final criticas =
        resumen?.criticas ??
        state.alertas.where((item) => item.alerta?.esCritica == true).length;
    final requierenRespuesta =
        resumen?.requierenConfirmacion ?? state.totalRequierenRespuesta;
...
class _FiltrosActivosSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (state.filtroRequiereConfirmacion != null) {
      chips.add(
        Chip(
          label: Text(
            state.filtroRequiereConfirmacion!
...
class _AlertaCard extends StatelessWidget {
  ...
  const _AlertaCard({required this.destinatario, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final noLeida = !destinatario.fueLeida;
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        if (alerta?.requiereConfirmacion == true)
                          _MiniLabel(
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            _formatearFecha(
                              alerta?.createdAt ?? destinatario.createdAt,
                            ),
                            style: Theme.of(context).textTheme.bodySmall
